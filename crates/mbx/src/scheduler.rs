...
///
/// Waking waiters on the release itself was measured and did not pay: a
/// build's aggregate idle here is a poll interval times its compilations,
/// which spread across the cores that are all compiling anyway comes to
/// about a second of a forty-second build. The pool is rarely what a build
/// is waiting on -- Cargo's own dependency graph is.
