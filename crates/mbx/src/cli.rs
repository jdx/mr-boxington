...
pub fn run() -> Result<ExitCode> {
    ...
    if let Commands::Doctor(args) = &cli.command {
        return crate::doctor::run_loaded(Config::load(), args.json, toolchain);
    }
    let (config, settings) = Config::load_for_cli()?;
    match cli.command {
        Commands::Doctor(_) => unreachable!("doctor was handled before configuration loading"),
        Commands::Explain(args) => crate::explain::run_with_settings(
            &config,
            &settings,
            &with_toolchain(toolchain, args.arguments()),
        ),
        ...
        Commands::Cargo(arguments) => {
            cargo(&config, &settings, &with_toolchain(toolchain, arguments))
        }
    }
}
