...
restore outputs without copying where they can. mbx prefers a reflink, whose
data blocks are shared until a file is modified, so writes to a restored output
never reach the cache object. Where the filesystem cannot clone -- ext4, most
importantly -- both fall back to a hard link to the stored object, which is
read-only so that a rewrite is refused rather than shared. mbx copies the bytes
only when it can do neither, and `restore_hardlink = false` asks it to copy
instead of linking.
