...
- **A crates.io Trusted Publisher for each of `mbx`, `mbx-cache-core`,
  ...
  workflow file `release-plz.yml`. Publishing uses OIDC, so no
  `CARGO_REGISTRY_TOKEN` exists anywhere. **Renaming `release-plz.yml` breaks
  publishing** until the trusted publishers are updated to match.
